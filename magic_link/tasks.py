...
@shared_task
def send_magic_link(email, expiry, token, url, external_name, expiry_date):
    template = get_email_template()
    send_mail(
        'Magic Link Request',
        template.format(url=url, token=token, expiry=expiry, external_name=external_name, expiry_date=expiry_date),
