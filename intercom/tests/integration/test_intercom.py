...
from intercom.intercom_api import reset_user_custom_attributes, post_issued_join_card_event, \
...
class IntercomApiTest(unittest.TestCase):
    ...
    def test_post_issued_join_card_event(self):
        ...
        # post a new event
        response_post = post_issued_join_card_event(self.TOKEN, self.USER_ID)
    ...
    def test_reset_custom_attributes(self):
        response = reset_user_custom_attributes(self.TOKEN, self.USER_ID)
        self.assertEqual(response.status_code, 200, response.text)
