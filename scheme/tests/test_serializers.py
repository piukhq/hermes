...
class TestAnswerValidation(GlobalMockAPITestCase):
    ...
    def test_date_of_birth_validation(self, mock_validate):
        serializer = JoinSerializer(data={"save_user_information": "true", "order": 0, "date_of_birth": "22/11/1999"})
        self.assertTrue(serializer.is_valid())
...
class TestSchemeSerializer(GlobalMockAPITestCase):
    ...
    def test_get_join_questions(self):
        ...
        question = SchemeCredentialQuestionFactory(
            type=BARCODE, scheme=scheme, options=SchemeCredentialQuestion.LINK_AND_JOIN, manual_question=True
        )
        question2 = SchemeCredentialQuestionFactory(
            type=LAST_NAME, scheme=scheme, options=SchemeCredentialQuestion.OPTIONAL_JOIN
        )
        ...
        link_question = SchemeCredentialQuestionFactory(
            type=FIRST_NAME, scheme=scheme, options=SchemeCredentialQuestion.LINK
        )
        none_question = SchemeCredentialQuestionFactory(
            type=TITLE, scheme=scheme, options=SchemeCredentialQuestion.NONE
        )
        merchant_question = SchemeCredentialQuestionFactory(
            type=CARD_NUMBER, scheme=scheme, options=SchemeCredentialQuestion.MERCHANT_IDENTIFIER
        )
        serializer = SchemeSerializer()
...
class TestUserConsentSerializer(GlobalMockAPITestCase):
    def setUp(self):
        ...
        self.consent1 = ConsentFactory(
            required=True, journey=JourneyTypes.LINK.value, check_box=True, scheme=self.scheme
        )
        self.consent2 = ConsentFactory(
            required=False, journey=JourneyTypes.LINK.value, check_box=True, scheme=self.scheme
        )
        self.consent3 = ConsentFactory(
            required=True, journey=JourneyTypes.JOIN.value, check_box=True, scheme=self.scheme
        )
        self.consent4 = ConsentFactory(
            required=True, journey=JourneyTypes.JOIN.value, check_box=True, scheme=self.scheme
        )
        self.consent5 = ConsentFactory(
            required=False, journey=JourneyTypes.JOIN.value, check_box=False, scheme=self.scheme
        )
    ...
    def test_get_user_consents(self):
        ...
        scheme_consents = Consent.objects.filter(
            scheme=self.scheme_account.scheme.id, journey=JourneyTypes.JOIN.value, check_box=True
        )
    ...
    def test_validate_consents_success(self):
        ...
        scheme_consents = Consent.objects.filter(scheme=self.scheme.id, journey=JourneyTypes.LINK.value, check_box=True)

        UserConsentSerializer.validate_consents(
            [self.user_consent1, self.user_consent2], self.scheme.id, JourneyTypes.LINK.value, scheme_consents
        )
    ...
    def test_validate_consents_raises_error_on_incorrect_number_of_consents_provided(self):
        ...
        self.assertEqual(
            "Incorrect number of consents provided for this scheme and journey type.", e.exception.detail["message"]
        )

        with self.assertRaises(serializers.ValidationError) as e:
            UserConsentSerializer.validate_consents(
                [self.user_consent1], self.scheme.id, JourneyTypes.LINK.value, scheme_consents
            )
...
class TestJoinSerializer(GlobalMockAPITestCase):
    ...
    def setUpTestData(cls):
        scheme = SchemeFactory()
        cls.req_question = SchemeCredentialQuestionFactory(
            type=BARCODE, scheme=scheme, options=SchemeCredentialQuestion.JOIN
        )
        cls.opt_request = SchemeCredentialQuestionFactory(
            type=LAST_NAME, scheme=scheme, options=SchemeCredentialQuestion.OPTIONAL_JOIN
        )
        context = {"scheme": scheme, "user": "1"}
