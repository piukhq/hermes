...
from ubiquity.tasks import (
    ...
    send_merchant_metrics_for_new_account, async_balance_with_updated_credentials,
)
...
class MembershipCardView(
    ...
    def replace(self, request, *args, **kwargs):
        ...
        return Response(
            self.get_serializer_by_request(
                account,
                context={"mcard_user_auth_provided_map": auth_provided_mapping}
            ).data, status=status.HTTP_200_OK
        )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if scheme_account.status in (SchemeAccount.JOIN_PENDING, SchemeAccount.REGISTRATION_ASYNC_IN_PROGRESS):
            # Ideally we would create a different message for pending registrations as this is a little misleading, but
            # avoiding non-agreed changes to the API for Barclays so will keep this the same for now.
    ...
    def _handle_create_link_route(
        ...
        if account_created and auth_fields:
            ...
            sch_acc_entry = SchemeAccountEntry.create_link(
                user, scheme_account, auth_provided=True
            )
