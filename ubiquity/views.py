...
class MembershipCardView(
    ...
    def get_queryset(self):
        return self.request.channels_permit.scheme_account_query(
            SchemeAccount.objects.select_related("scheme").prefetch_related("scheme__schemeoverrideerror_set"), user_id=self.request.user.id, user_filter=True
        )
    ...
    def _handle_create_join_route(
        ...
        if scheme.slug in plr_slugs:
            ...
            if other_accounts.exists():
                ...
                SchemeAccountEntry.create_link(scheme_account=scheme_account, user=user)
...
class ListMembershipCardView(MembershipCardView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            self.get_serializer_by_request(account, context={"request": request}).data,
            status=status_code
        )
