...
from scheme.models import Scheme, SchemeBundleAssociation
...
class TestPermit(GlobalMockAPITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.bink_scheme_bundle_association = SchemeBundleAssociationFactory(
            scheme=cls.bink_scheme, bundle=cls.bink_bundle, status=SchemeBundleAssociation.INACTIVE
        )
        ...
        cls.ubiquity_client_app = ClientApplicationFactory(
            organisation=cls.ubiquity_organisation, name="ubiquity client application", client_id=cls.UBIQUITY_CLIENT_ID
        )

        cls.ubiquity_bundle = ClientApplicationBundleFactory(
            bundle_id=cls.UBIQUITY_BUNDLE_ID, client=cls.ubiquity_client_app
        )

        cls.ubiquity_scheme = SchemeFactory()
        cls.ubiquity_scheme_bundle_association = SchemeBundleAssociationFactory(
            scheme=cls.ubiquity_scheme, bundle=cls.ubiquity_bundle, status=SchemeBundleAssociation.ACTIVE
        )
    ...
    def test_scheme_query_both_active(self):
        ...
        ubiquity_permit = self.set_ubiquity_status(
            self.ubiquity_scheme_bundle_association, SchemeBundleAssociation.ACTIVE
        )
    ...
    def test_scheme_query_bink_active_ubiquity_inactive(self):
        bink_permit = self.set_status(self.bink_scheme_bundle_association, SchemeBundleAssociation.ACTIVE)
        ubiquity_permit = self.set_ubiquity_status(
            self.ubiquity_scheme_bundle_association, SchemeBundleAssociation.INACTIVE
        )
    ...
    def test_scheme_query_bink_active_ubiquity_suspended(self):
        bink_permit = self.set_status(self.bink_scheme_bundle_association, SchemeBundleAssociation.ACTIVE)
        ubiquity_permit = self.set_ubiquity_status(
            self.ubiquity_scheme_bundle_association, SchemeBundleAssociation.SUSPENDED
        )
...
class TestPermitSharedScheme(GlobalMockAPITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.bink_scheme_bundle_association = SchemeBundleAssociationFactory(
            scheme=cls.scheme, bundle=cls.bink_bundle, status=SchemeBundleAssociation.INACTIVE
        )
        ...
        cls.ubiquity_scheme_bundle_association = SchemeBundleAssociationFactory(
            scheme=cls.scheme, bundle=cls.ubiquity_bundle, status=SchemeBundleAssociation.ACTIVE
        )
...
class TestInternalService(GlobalMockAPITestCase):
    ...
    def setUpTestData(cls):
        ...
        SchemeBundleAssociationFactory(
            scheme=cls.scheme, bundle=cls.other_bundle, status=SchemeBundleAssociation.ACTIVE
        )
    ...
    def test_service_get_any_payment_card_account(self):
        ...
        resp = self.client.get(reverse("payment-card", args=[self.payment_card_account_1.id]), **self.auth_headers)
        self.assertEqual(resp.status_code, 404)

        resp = self.client.get(reverse("payment-card", args=[self.payment_card_account_2.id]), **self.auth_headers)
        self.assertEqual(resp.status_code, 200)
