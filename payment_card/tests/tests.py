...
class TestPaymentCard(APITestCase):
    ...
    def test_payment_card_list(self):
        response = self.client.get('/payment_cards/')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(type(response.data), ReturnList)
        self.assertTrue(response.data)

    def test_get_payment_card_account(self):
        response = self.client.get('/payment_cards/accounts/{0}'.format(self.payment_card.id))
    ...
    def test_post_payment_card_account(self):
        ...
        response = self.client.post('/payment_cards/accounts', data)
    ...
    def test_patch_payment_card_account(self):
        data = {'pan': '345345345'}
        response = self.client.patch('/payment_cards/accounts/{0}'.format(self.payment_card_account.id), data=data)
