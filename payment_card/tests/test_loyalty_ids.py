...
class TestRetrieveLoyaltyID(GlobalMockAPITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.scheme_account_entry_1 = ubiquity.tests.factories.SchemeAccountEntryFactory(user=cls.user_1, scheme_account=cls.scheme_account_1)
        cls.scheme_account_entry_2 = ubiquity.tests.factories.SchemeAccountEntryFactory(user=cls.user_2, scheme_account=cls.scheme_account_2)
    ...
    def test_404_scheme_unavailable(self):
        response = self.client.post(
            "/payment_cards/accounts/loyalty_id/{}".format("unavailable_scheme"),
            json.dumps(
                {"payment_cards": [self.payment_card_account_1.psp_token, self.payment_card_account_2.psp_token]}
            ),
            content_type="application/json",
